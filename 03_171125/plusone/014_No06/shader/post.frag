...
void main() {
	gl_FragColor = nightScope(texture, vTexCoord);

	//色収差をつけてみる(https://ja.wikipedia.org/wiki/%E8%89%B2%E5%8F%8E%E5%B7%AE)
	const vec2 offset = vec2(0.01, 0.0);
	gl_FragColor.r = nightScope(texture, vTexCoord - offset).r * 1.5;
	gl_FragColor.b = nightScope(texture, vTexCoord + offset).b * 1.5;
}

...
(() => {
    ...
    window.addEventListener('load', () => {
        ...
        createTexture('./image/background.jpg', (texture0) => {
    ...
    function init(texture){
        ...
        scenePrg.uniLocation[1] = gl.getUniformLocation(scenePrg.program, 'texture');
        ...
        let VBO = [
            createVbo(sphereData.p),
            createVbo(sphereData.t)
        ];
    ...
    class InteractionCamera {
        ...
        constructor(){
            ...
            this.rotateX           = 0.0;
            this.rotateY           = 0.0;
            this.rotateXPower      = 0.0;
            this.rotateYPower      = 0.0;
            this.rotateAttenuation = 0.8;
            this.scale             = 1.0;
            this.scalePower        = 0.0;
        ...
        moveEvent(eve){
            ...
            let w = canvasWidth;
            let h = canvasHeight;
            let s = 1.0 / Math.min(w, h) / 2.0;
            let x = eve.clientX - this.prevMouse[0];
            let y = eve.clientY - this.prevMouse[1];
            this.rotateXPower = x * s;
            this.rotateYPower = y * s;
            this.prevMouse = [eve.clientX, eve.clientY];
        }
        /**
         * mouse up event
        ...
        update(){
            ...
            this.rotateXPower *= this.rotateAttenuation;
            this.rotateYPower *= this.rotateAttenuation;
            this.rotateX -= this.rotateXPower;
            if(this.rotateX < 0.0){
                this.rotateX = 1.0 + this.rotateX;
            }
            this.rotateY -= this.rotateYPower;
            this.rotateY = Math.min(Math.max(this.rotateY, -0.25), 0.25);
            let qx = qtn.identity(qtn.create());
            let qy = qtn.identity(qtn.create());
            qtn.rotate(-this.rotateX * Math.PI * 2.0, [0.0, 1.0, 0.0], qx);
            qtn.rotate(this.rotateY * Math.PI * 2.0, [1.0, 0.0, 0.0], qy);
            qtn.multiply(qx, qy, this.qtn);
        }
    }
